...
class BaseController extends Controller {
     ...
     * 服务联盟图片上传
     */
    public function uploadServicePhoto() {
        ...
        $sizeArr = C('THUMB_SIZE');
        $thumb =[
            'thumb' =>  $sizeArr[202] ? $sizeArr[202] : array(100, 100),
        ];
        $arr =$this->_upload('service/', $thumb);
        if ($arr) {
            $data = [
                'service_id' => $service_id,
                'thumb'       => $arr['path']['thumb'],
            ];
            if (M('ServicePhoto')->add($data));
            $arr = ['status' => 1, 'thumb' => $data['thumb']];
        }
        echo json_encode($arr);
    }
